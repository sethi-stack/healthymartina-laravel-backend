...
- Shopping list management & PDF export
...
### Phase C: Shopping List Management (Lines 405-674)
**Priority: HIGH** - Critical feature

#### Methods to Migrate:
1. **`calendarioLista()` (lines 405-436)** - View shopping list page
   - Gets calendar's shopping list
...
4. **`calendarioListaPdf()` (lines 514-589)** - 🔥 **SHOPPING LIST PDF EXPORT**
...
6. **`ListaEmail()` (lines 611-638)** - Email shopping list (HTML)
...
**API Endpoints Needed:**
- `GET /api/v1/calendars/{id}/shopping-list` - Get shopping list
- `GET /api/v1/calendars/{id}/shopping-list/categories` - Get by category
- `GET /api/v1/calendars/{id}/shopping-list/pdf` - Download PDF
- `POST /api/v1/calendars/{id}/shopping-list/pdf/email` - Email PDF
- `POST /api/v1/calendars/{id}/shopping-list/toggle-taken` - Mark taken
- `POST /api/v1/calendars/{id}/shopping-list/custom` - Add custom item
- `PUT /api/v1/calendars/{id}/shopping-list/custom/{itemId}` - Update item
- `DELETE /api/v1/calendars/{id}/shopping-list/custom/{itemId}` - Delete item
...
### **PHASE 1: Shopping List (Critical)** ⭐⭐⭐
Lines 405-674 (9 methods)
- Most used feature after recipes
...
│   ├── ShoppingListController.php ⚠️ (NEW)
│   └── ShoppingListPdfController.php ⚠️ (NEW)
│
├── Plans/
...
1. **Create ShoppingListController** - Start with Phase 1
2. **Create ShoppingListPdfController** - Handle PDF generation
3. **Test with existing database** - Shopping list is most critical
